...
| Hidden-information advantage | The passive observer reads visible duel-log text and the renderer's `public_log` field only. It never reads `private_log`. Opt-in custom macros may read the current player's own DuelingBook arrays to locate named cards, but never opponent hidden-zone arrays. |
