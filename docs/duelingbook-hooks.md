...
| `#deck_constructor img.deck_constructor` | Deck-editor background image | Reversible visual theme | Preserve and replace both `src` and lazy `data-src`; restore both when disabled. |
| `#search > img` | Deck-editor search background image | Reversible visual theme | Preserve and replace both `src` and lazy `data-src`; pair it with readable light labels. |
