...
| `#duel #field .card`, jQuery `face_down` data | Public duel field cards | Face-up rarity presentation | Apply saved treatments only when DuelingBook explicitly reports `face_down` as false; clean overlays immediately from every other duel cardfront. |
| `.cardfront`, jQuery `pendulum` data | Public card template metadata | Pendulum-shaped rarity asset selection | `2` selects small, `1`/`3` normal, `4` large; no gameplay state is changed. |
