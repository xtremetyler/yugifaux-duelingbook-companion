...
| `#deck_constructor`, `.deck_bg`, `.side_bg`, `.extra_bg` | Current deck-editor background layers | Reversible visual theme | Apply the themed SVG to the container and make only its native colored panels transparent; restore by removing the body theme class. |
| `#search`, `.search_bg` | Current deck-editor search background layers | Reversible visual theme | Apply the themed SVG to the container, make its native green panel transparent, and pair it with readable light labels. |
